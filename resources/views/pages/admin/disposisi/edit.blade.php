                    ...
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <input class ="form-control" type="text" name="letter_id" value={{ $item->letter->letter_no }} disabled>
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <div class="form-check form-check-inline mt-4">
                                        <input class="form-check-input" type="radio" name="status_surat" id="inlineRadio1" value="0" {{ $item->status_surat == 'Disposisi' ? 'checked' : '' }}>
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control single-select-field" id="asal_unit" data-placeholder="Pilih Direksi/Karyawan" required>
                                        {{-- <option>== Pilih Unit ==</option> --}}
                                            ...
                                            <option value="{{ $department->id }}" {{ ($item->asalDisposisi->department->id == $department->id)? 'selected':''; }}>{{ $department->name }}</option>
                                            @endforeach
                                    </select>
                                </div>
                                @error('department_id')
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control single-select-field" id="asal_direksi_karyawan" data-placeholder="Pilih Direksi/Karyawan" required>
                                        <option>== Pilih Direksi/Karyawan ==</option>
                                            ...
                                            <option value="{{ $employee->id }}" {{ ($item->asalDisposisi->nama == $employee->nama)? 'selected':''; }}>{{ $employee->nama }}</option>
                                            @endforeach --}}
                                    </select>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control single-select-field" id="tujuan_unit" data-placeholder="Pilih Unit" required>
                                        <option>== Pilih Unit ==</option>
                                            @foreach ($departments as $department)
                                            <option value="{{ $department->id }}" {{ ($item->tujuanDisposisi->department->id == $department->id)? 'selected':''; }}>{{ $department->name }}</option>
                                            @endforeach
                                    </select>
                                </div>
                                @error('department_id')
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control single-select-field" id="tujuan_direksi_karyawan" data-placeholder="Pilih Direksi/Karyawan" required>
                                        <option>== Pilih Direksi/Karyawan ==</option>
                                            {{-- @foreach ($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ ($item->tujuanDisposisi->nama == $employee->nama)? 'selected':''; }}>{{ $employee->nama }}</option>
                                            @endforeach --}}
                                    </select>
                                </div>
                            </div>
                            <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                    <textarea id="isi_disposisi" class="form-control @error('isi_disposisi') is-invalid @enderror" name="isi_disposisi" placeholder="Isi Disposisi/Eskalasi.." required>{{ old('catatan_rektor', $item->isi_disposisi) }}</textarea>
                                </div>
                                @error('isi_disposisi')
                                <div class="invalid-feedback">
                                    {{ $message; }}
...
<script>
    ...
    $(document).ready(function(){
        $('#asal_unit').change(function() {
            ...
            if (id) {
                $.ajax({
                    type: "GET",
                    url: '{{ route("getDropdownKaryawan", ":asal_unit") }}'.replace(':asal_unit', id),
                    dataType: 'JSON',
                    success: function(response) {
    ...
    $(document).ready(function() {
        ...
        function loadEmployeesByDepartment(asal_unit) {
            $.ajax({
                type: 'GET',
                url: '{{ route("getDropdownKaryawan", ":asal_unit") }}'.replace(':asal_unit', asal_unit),
                dataType: 'json',
                success: function(response) {
                    ...
                    $.each(response.karyawan, function(index, emp) {
                        $('#asal_direksi_karyawan').append($('<option>', {
                            value: emp.id,
                            text: emp.nama,
                            selected: emp.id == "{{ $item->asal_disposisi }}"
                        }));
                    });
                }
            });
    ...
    $(document).ready(function(){
        $('#tujuan_unit').change(function() {
            ...
            if (id) {
                $.ajax({
                    type: "GET",
                    url: '{{ route("getDropdownKaryawan", ":tujuan_unit") }}'.replace(':tujuan_unit', id),
                    dataType: 'JSON',
                    success: function(response) {
                        // Kosongkan dropdown sebelum menambahkan opsi baru
    ...
    $(document).ready(function() {
        ...
        function loadEmployeesByDepartment(tujuan_unit) {
            $.ajax({
                type: 'GET',
                url: '{{ route("getDropdownKaryawan", ":tujuan_unit") }}'.replace(':tujuan_unit', tujuan_unit),
                dataType: 'json',
                success: function(response) {
                    // $('#karyawandireksi_pengirim').empty(); // Kosongkan dropdown karyawan
                    console.log(response);
                    ...
                    $.each(response.karyawan, function(index, emp) {
                        $('#tujuan_direksi_karyawan').append($('<option>', {
                            value: emp.id,
                            text: emp.nama,
                            selected: emp.id == "{{ $item->tujuan_disposisi}}"
                        }));
                    });
                }
            });
